...
{aura_info}
   Each image 190px tall, object-fit cover, grayscale. Below: card number monospace 10px #c0bdb5, caption monospace 11px #8a857d — use the descriptions above to write what you see + why chosen for that transit. If no description is available, write a brief evocative caption that reflects the day's theme.
...
12. EROS CAROUSEL — 3 portrait cards, 200px wide, 320px tall images, grayscale opacity 1.0. Label this section "EROS" in the small monospace header. Images:
    ...
    WHAT IS IN EACH IMAGE:
{eros_info}
    Each card: number title + caption using the descriptions above — what you actually see + why chosen for this transit/placement. If no description is available, write a brief evocative caption reflecting Venus or the sensual current of the day.
...
16. TODAY'S TOTEM — A single centered send-off image acting as the day's ally. Outer container: padding 32px 24px, border-top 1px solid #e0ddd5, text-align center.
    ...
    Below the image: a reasoning caption in italic 14px #1f1d18, line-height 1.7, max-width 360px, margin auto, padding-top 14px. 1-2 sentences explaining why this image is today's totem — connect what's visibly in the image to today's transits/dasha/theme.
    Image metadata to reason from:
{totem_info}
    If metadata description is empty, derive the reasoning from the image URL alone using evocative, atmospheric language tied to today's astrology — keep it brief and grounded.
