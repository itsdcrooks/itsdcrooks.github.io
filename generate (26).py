...
# Parse --recipient flag (or RECIPIENT env var). Defaults to "daniel" for back-compat.
_parser = argparse.ArgumentParser()
_parser.add_argument("--recipient", default=os.environ.get("RECIPIENT", "daniel"),
                     help="Recipient ID matching a key in recipients.json")
_args, _ = _parser.parse_known_args()
RECIPIENT_ID = _args.recipient

# Load recipient config
_recipients_path = "recipients.json"
if not os.path.exists(_recipients_path):
    raise FileNotFoundError(f"recipients.json missing — required for config-driven generation")
with open(_recipients_path) as _f:
    _all_recipients = json.load(_f)
if RECIPIENT_ID not in _all_recipients:
    raise ValueError(f"Recipient '{RECIPIENT_ID}' not found in recipients.json. "
                     f"Available: {sorted(_all_recipients.keys())}")
RECIPIENT = _all_recipients[RECIPIENT_ID]
print(f"[recipient] generating for: {RECIPIENT_ID} ({RECIPIENT['display_name']})")

ANTHROPIC_API_KEY  = os.environ["ANTHROPIC_API_KEY"]
GMAIL_ADDRESS      = os.environ.get("GMAIL_ADDRESS", "")
GMAIL_APP_PASSWORD = os.environ.get("GMAIL_APP_PASSWORD", "")

# Per-recipient constants pulled from config
DISPLAY_NAME       = RECIPIENT["display_name"]
FORMAL_NAME        = RECIPIENT.get("formal_name", DISPLAY_NAME)
TO_EMAIL           = os.environ.get(RECIPIENT["email_secret"], "")
if not TO_EMAIL:
    raise RuntimeError(f"Missing email env var '{RECIPIENT['email_secret']}' for recipient '{RECIPIENT_ID}'")
GITHUB_PAGES_URL   = RECIPIENT["page_url"].rstrip("/")
OUTPUT_PATH        = RECIPIENT["output_path"]
LEGACY_OUTPUT_PATH = RECIPIENT.get("legacy_output_path")
USED_IMAGES_PATH   = RECIPIENT["used_images_path"]
USED_QUOTES_PATH_CFG = RECIPIENT["used_quotes_path"]
INTRO_DESCRIPTOR   = RECIPIENT["intro_descriptor"]
NAMING_RULE        = RECIPIENT["naming_rule"]
TOPBAR_DEFAULT     = RECIPIENT["topbar_default"]
VEDIC_CHIPS        = RECIPIENT["vedic_chips"]
EMAIL_SUBJECT_TPL  = RECIPIENT["email_subject_template"]
EMAIL_BODY_TPL     = RECIPIENT["email_body_template"]
COMMIT_MSG_TPL     = RECIPIENT["commit_message_template"]
...
WESTERN_CHART = RECIPIENT["western_chart"]

VEDIC_CHART = RECIPIENT["vedic_chart"]
...
USED_IMAGES_FILE = USED_IMAGES_PATH
...
def generate_horoscope(planet_data, images, todays_quotes=None):
    ...
    prompt = f"""You are generating a daily personal horoscope for {DISPLAY_NAME} — {INTRO_DESCRIPTOR}. Today is {date_str} ({day_name}). THIS READING IS SPECIFICALLY FOR {date_str} — do not reuse content from any previous date.
...
1. TOPBAR — date left, "{TOPBAR_DEFAULT}" right (use today's actual transiting Sun sign + the natal Ascendant from the chart above; the default shown here is a fallback). Monospace 11px #8a857d letter-spacing 0.18em. Border-bottom 1px solid #e0ddd5. Padding 26px 24px 14px.
...
DIRECTNESS RULES (CRITICAL — apply throughout this section):
- Declarative sentences. State the observation; do not pad it.
- BANNED constructions (these are AI-pattern tells): "It's not X, it's Y" / "This isn't just X, it's Y" / "Not just X but Y" / "not X but Y" (any form — "not a contradiction to manage but a polarity to inhabit" is banned; "not a task but a doorway" is banned; the whole family of "not X but Y" is banned regardless of what fills X and Y) / "Less X, more Y" / "What this really is, is Y" / "X, not Y" as a reversal / "rather than Y" as a false-choice pivot. Never use these.
- NO TRIPLE LISTS. When making a point, do not illustrate it with three parallel examples (e.g. "the friendships, the networks, the collective identifications"). Pick one specific example, or none. Trust the reader to fill in the field. If you find yourself writing a comma-separated list of three abstractions, delete two of them.
- ONE POINT PER SENTENCE. If a sentence carries two ideas linked by "and," "which makes," "so that," or an em-dash — break it into two sentences. Vary sentence length: at least one sentence per paragraph should be under six words. Rhythm is: statement, statement, breath. Not: statement-and-example-and-qualification.
- Cut hedging fillers: avoid "perhaps", "may", "could", "might", "in some sense", "in a way" unless the qualification is genuinely necessary.
- Cut filler clauses: no "in a way that", "in such a manner that", "something like", "what feels like a kind of".
- No meta-commentary on the chart or reading: avoid "This is significant because...", "What's interesting here is...", "It's worth noting...".
- Trust the reader. Do not restate the same insight twice. Do not summarize what you just said.
- State, don't soften.

STYLE MODELS. The voice should sit at the intersection of directness and music — short declarative sentences with weight, occasionally punctuated by a longer rhythmic breath. Model the cadence on:
- Joan Didion's essays: spare, exact, rhythmically varied. She lets a paragraph end on a small blunt sentence.
- James Baldwin's essays: declarative moral clarity. Weight without decoration. Sentences that land like verdicts.
- Cormac McCarthy: biblical cadence. Almost no commas where they aren't needed. Statements arrive like weather.
- John Berger: philosophical directness. Everyday nouns doing metaphysical work. No adjective-stacking.
- Marguerite Duras: spare, hypnotic. Repetition used deliberately for emphasis. Silence around each sentence.

Elegance comes from concrete nouns and rhythmic variation, not from adjective-stacking or abstract phrases. "The body wants light" is stronger than "the somatic self yearns toward luminosity." Prefer the shorter Anglo-Saxon word to the longer Latinate one when both are available. Prefer a specific noun (the wolf, the hearth, the door) to an abstract category (the animal presence, the domestic sphere, the threshold experience).
...
7. PLANET STATUS — 10 planets. Section header: "PLANET STATUS" (small monospace uppercase). Cols: symbol (14px monospace #8a857d, 28px wide) | name + house + sign (10px monospace #555, ~150px wide — REQUIRED FORMAT for every row: "[NAME] [house]H · [SIGN]" e.g. "SUN 6H · CANCER", "MOON 2H · PISCES", "MERCURY 6H · CANCER". Both house AND sign must appear on every row — non-negotiable, no exceptions, no alternative formats like "SUN · 6TH" alone) | status word (active=#4a7a3a / challenged=#a05540 / dormant=#8a857d) + explanation (13px #1f1d18). Row border-bottom 1px solid #e8e5dd.
...
11. SENSUAL SECTION — label "Venus in [sign] · [Moon situation]". 3 paragraphs, font-size 17px, line-height 1.82 #1f1d18. Embed the SECTION 11 QUOTE provided above inside the paragraphs as <em>...</em> (italic #0a0a0a) with full attribution. Do not substitute another quote.
   Apply DIRECTNESS RULES from Section 5 throughout — no false-contrast constructions, no hedging fillers, no meta-commentary. Direct declarative sentences.
...
14. THE LESSON — centered, font-size 21px italic #0a0a0a, max-width 380px margin auto. 2-3 sentences specific to {DISPLAY_NAME}'s Western chart.
...
15. TRY THIS — two-column grid (1fr 1fr, gap 2px, background #d4d1ca). Each cell background #faf8f3, padding 22px 18px. Label monospace 10px #8a857d + text 14px #1f1d18.
    VOICE — READ CAREFULLY. These are invitations to notice, not instructions to perform. The person already has their practices — the morning ceremony, the client sessions, the journaling, the movement, the meditation. Do not tell them to do a practice they already do. Instead, point their attention toward something specific WITHIN the practice they will already be inside, and invite them to notice what arises.

    STRUCTURE — each suggestion should have two parts:
      (1) Ground the moment in the astrology and the practice they already have — the scene the reader is already inside.
      (2) End with an open question or a specific noticing invitation. Never end with a directive. What arose? What softened? What insisted on being seen? What did the body know before the mind caught up?

    Never open a suggestion with a bare imperative verb (Perform... Do... Take... Practice... Sit... Try... Make...). The imperative-verb opening is the AI-pattern to-do-list tell.

    Weak (avoid): "Perform one tea ceremony in silence before speaking."
    Strong (aim for): "The Capricorn Moon closes the night's accounting through gesture, water, heat. When you sit with the pot this morning — what arrives in the silence before the first word?"

    Weak: "Sit with your journal open. Write the idea that arrives."
    Strong: "The Aquarius Moon crosses your Uranus this afternoon. When you're at the page today, notice which idea comes fully formed — and which one you were about to edit down. What did the smaller version protect you from?"

    Weak: "Take a walk at sunset."
    Strong: "The hour before the Sun goes down carries a different pressure. If you walk today, notice what the body knows about the day before the mind gives it words. What sentence would that knowing use?"

    Weak: "During your session today, hold space for the client."
    Strong: "Mercury conjunct your Sun today means clinical language will carry unusual charge. In session, notice the moment the client's language shifts — what did they say just before it did?"

    Ground each suggestion in the actual transit, dasha, or natal placement it responds to — name the specific astrology. Two suggestions per reading: one anchored to the morning or early day, one anchored to a specific later transit with an exact time when possible.
...
16. TODAY'S TOTEM — A single centered send-off image acting as the day's ally. Outer container: padding 32px 24px, border-top 1px solid #e0ddd5, text-align center.
    ...
    FORMAT (3-4 sentences, each doing distinct work):
      Sentence 1 — Concrete visual: name the specific subject from the metadata description (animal, deity, figure, scene, composition). Pull from the description close to verbatim. LEAD WITH THE SUBJECT; do not open with abstract phrases like "a presence", "a form", "a figure at the edge of".
      Sentence 2 — Astrological tie: which specific transit, dasha lord, or nakshatra makes this image today's totem, and why this subject answers it.
      Sentence 3 — Teaching: what the totem asks of the reader today — a practical orientation, posture, or attention quality.
      Optional Sentence 4 — Deeper resonance: a brief mythological, symbolic, or psychological echo that grounds the totem in something larger.
    Apply the same DIRECTNESS RULES from the main reading (no "it's not X, it's Y" constructions, no hedging, no meta-commentary).
    Image metadata:
{totem_info}
    If metadata description is genuinely empty, write minimal: "Today's totem — chosen for [specific transit]" — do not invent visual details you cannot verify.
...
18. VEDIC CHIPS — {VEDIC_CHIPS}. Same chip style as Western.
...
20. VEDIC PLANET STATUS — same format as Section 7. Section header: "VEDIC PLANET STATUS". REQUIRED FORMAT for every row: "[NAME] [house]H · [SIDEREAL SIGN]" e.g. "SUN 7H · GEMINI", "MOON 11H · LIBRA", "RAHU 5H · ARIES". Both house AND sidereal sign must appear on every row — non-negotiable. Include Rahu and Ketu.
...
23. VEDIC LESSON — centered italic 21px #0a0a0a, 2 sentences. Karmic/dharmic insight specific to {DISPLAY_NAME}'s Vedic chart.
...
"""
    ...
    # Streaming request with retry — necessary for max_tokens=32000.
    # Anthropic occasionally drops long streaming connections; retry up to 3 times.
...
def wait_until_5am_central():
    """Sleep until 5:00 AM US Central (handles DST automatically) for consistent delivery."""
    try:
        from zoneinfo import ZoneInfo
    except ImportError:
        from backports.zoneinfo import ZoneInfo  # Py 3.8 fallback
    central = ZoneInfo("America/Chicago")
    now = datetime.now(central)
    target = now.replace(hour=5, minute=0, second=0, microsecond=0)
    if now >= target:
        print(f"[wait] past 5:00 AM Central already ({now.strftime('%H:%M %Z')}); sending now.")
        return
    wait_seconds = (target - now).total_seconds()
    mins = int(wait_seconds // 60)
    print(f"[wait] sleeping {mins} min until 5:00 AM Central for consistent delivery...")
...
def send_email(url):
    ...
    msg["Subject"] = EMAIL_SUBJECT_TPL.format(date=today, url=url)
    msg["From"] = GMAIL_ADDRESS
    msg["To"] = TO_EMAIL
    body = EMAIL_BODY_TPL.format(date=today, url=url)
...
USED_QUOTES_PATH = USED_QUOTES_PATH_CFG
...
def main():
    ...
    if LEGACY_OUTPUT_PATH:
        print(f"Saving {LEGACY_OUTPUT_PATH} (compatibility)...")
        _legacy_dir = os.path.dirname(LEGACY_OUTPUT_PATH)
        if _legacy_dir:
