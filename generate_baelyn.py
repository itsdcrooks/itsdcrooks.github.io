...
GITHUB_PAGES_URL   = "https://itsdcrooks.github.io/baelyn/"
...
WESTERN_CHART = """
Sun in Virgo, 4°09' — 10th House
Moon in Sagittarius, 19°06' — 2nd House
Ascendant in Scorpio, 8°16' — 1st House
Midheaven in Leo, 13°19' — 10th House cusp
Mercury in Virgo, 29°31' — 11th House
Venus in Leo, 16°17' — 10th House
Mars in Scorpio, 14°39' — 1st House (conjunct Ascendant)
Jupiter in Scorpio, 5°28' — 12th House
Saturn in Libra, 19°15' — 12th House
Uranus in Sagittarius, 0°43' — 1st House
Neptune in Sagittarius, 24°17' — 2nd House
Pluto in Libra, 24°55' — 12th House
"""

VEDIC_CHART = """
Ascendant: Libra (Tula), 14°39' — ruled by Venus — Swati nakshatra (Rahu-ruled)
Sun: Leo (Simha), 10°32' — Magha nakshatra (Ketu-ruled) — 11th House
Moon: Scorpio (Vrishchika), 25°29' — Jyeshtha nakshatra (Mercury-ruled) — 2nd House
Mars: Libra, 21°02' (debilitated) — Vishakha nakshatra — 1st House (conjunct ASC)
Mercury: Virgo (own sign), 5°54' — Uttara Phalguni nakshatra — 12th House
Jupiter: Libra, 11°51' — Swati nakshatra — 1st House (conjunct ASC, in Swati like the Lagna)
Venus: Cancer, 22°40' — Ashlesha nakshatra — 10th House
Saturn: Virgo, 25°38' — Chitra nakshatra — 12th House
Rahu: Gemini, 16°55' — Ardra nakshatra — 9th House
Ketu: Sagittarius, 16°55' — Purva Ashadha nakshatra — 3rd House

Current Vimshottari Dasha: Moon Mahadasha (May 29, 2021 — May 29, 2031)
Current Antardasha: Moon-Saturn (Aug 28, 2025 — March 29, 2027)
"""
...
USED_IMAGES_FILE = "used_images_baelyn.json"
...
def generate_horoscope(planet_data, images, todays_quotes=None):
    ...
    prompt = f"""You are generating a daily personal horoscope for Baelyn Elspeth — a practitioner of the ceremonial arts (The Way of Tea, Zen tradition), a writer, and a dancer. She is grounded in divine feminine embodiment, sacred ritual, depth psychology, and Vedic mysticism. The reading should honor all of these registers — esoteric, embodied, contemplative, sacred — and assume she meets symbolic and mystical language at full depth. Today is {date_str} ({day_name}). THIS READING IS SPECIFICALLY FOR {date_str} — do not reuse content from any previous date.

NAMING RULE:
- Default to "Baelyn" when addressing her in the readings.
- Shift to "Baelyn Elspeth" only on solemn moments — the deepest invocation, the sacred address, the ritual frame. The longer form is reserved.
- Use her name sparingly — no more than three total mentions across the page.
...
1. TOPBAR — date left, "☀ Virgo · ↑ Scorpio" right. Monospace 11px #8a857d letter-spacing 0.18em. Border-bottom 1px solid #e0ddd5. Padding 26px 24px 14px.
...
- BANNED constructions (these are AI-pattern tells): "It's not X, it's Y" / "This isn't just X, it's Y" / "Not just X but Y" / "Less X, more Y" / "What this really is, is Y". Never use these.
...
14. THE LESSON — centered, font-size 21px italic #0a0a0a, max-width 380px margin auto. 2-3 sentences specific to Baelyn's Western chart.
...
18. VEDIC CHIPS — Libra ASC · Leo Sun · Scorpio Moon · Moon Dasha · Jupiter Conjunct Lagna · Mars Conjunct Lagna. Same chip style as Western.
...
23. VEDIC LESSON — centered italic 21px #0a0a0a, 2 sentences. Karmic/dharmic insight specific to Baelyn's Vedic chart.
...
def send_email(url):
    ...
    msg["Subject"] = f"☽ Daily Reading for Baelyn · {today}"
    msg["From"] = GMAIL_ADDRESS
    msg["To"] = TO_EMAIL
    body = f"Baelyn — your daily reading for {today} is ready.\n\nOpen it here: {url}\n\n☽"
...
USED_QUOTES_PATH = "used_quotes_baelyn.json"


def main():
    print(f"Generating Baelyn's horoscope for {datetime.now().strftime('%B %d, %Y')}...")
    ...
    # Write to baelyn/ (where GitHub Pages serves Baelyn's page from)
    print("Saving baelyn/index.html...")
    os.makedirs("baelyn", exist_ok=True)
    with open("baelyn/index.html", "w") as f:
        f.write(html)

    # Also write to docs/baelyn/ so any older workflow that still references that
    # path doesn't fail on git add.
    print("Saving docs/baelyn/index.html (compatibility)...")
    os.makedirs("docs/baelyn", exist_ok=True)
    with open("docs/baelyn/index.html", "w") as f:
        f.write(html)
    ...
    try:
        ...
        subprocess.run(
            ["git", "add", "baelyn/index.html", "docs/baelyn/index.html"],
            check=False,
        )
        ...
        if diff.returncode != 0:
            ...
            subprocess.run(
                ["git", "commit", "-m", f"Daily horoscope Baelyn {today}"],
                check=False,
            )
