...
def main():
    ...
    # Step 2: Remove sensual entries AND any other images depicting women
    # The user wants the 90 uploaded URLs to be the ONLY women images in the catalogue.
    WOMEN_PATTERN = re.compile(
        r"\b(woman|women|female|girl|girls|lady|ladies|femme|feminine|wife|daughter|mother|bride|nun|ballerina)\b"
        r"|\bher (face|hair|body|hand|hands|skin|eyes|gaze|lips|back|legs|figure)\b",
        re.IGNORECASE,
    )
    ...
    def is_woman_entry(e):
        if not e.get("is_figure"):
            return False
        blob = (e.get("description", "") or "") + " " + " ".join(e.get("subjects", []) or [])
        return bool(WOMEN_PATTERN.search(blob))

    def should_remove(e):
        return is_sensual_entry(e) or is_woman_entry(e)

    before = len(catalogue)
    n_sensual = sum(1 for e in catalogue if is_sensual_entry(e))
    n_women_extra = sum(1 for e in catalogue if not is_sensual_entry(e) and is_woman_entry(e))
    new_catalogue = [e for e in catalogue if not should_remove(e)]
    removed = before - len(new_catalogue)
    print(f"Removed {removed} entries total:")
    print(f"  • {n_sensual} sensual-tagged (is_sensual=true or mood=sensual)")
    print(f"  • {n_women_extra} additional women-depicting entries")
    print(f"Catalogue now has {len(new_catalogue)} entries (before adding the new 90).")
